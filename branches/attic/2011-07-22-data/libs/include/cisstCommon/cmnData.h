...
#if 0
#define CMN_DATA_BYTE_SWAP_16_MACRO(x) *(unsigned short *)&(x) = ( ((*(unsigned short *)&(x) & 0xff) << 8) | \
                                                                   (*(unsigned short *)&(x) >> 8) )
#define CMN_DATA_BYTE_SWAP_32_MACRO(x) *(unsigned int *)&(x) = ( (*(unsigned int *)&(x) << 24) | \
                                                                 ((*(unsigned int *)&(x) << 8) & 0x00ff0000) | \
                                                                 ((*(unsigned int *)&(x) >> 8) & 0x0000ff00) | \
                                                                 (*(unsigned int *)&(x) >> 24) )
#define	CMN_DATA_BYTE_SWAP_64_MACRO(x) *(unsigned long long int *)&(x) = ( (*(unsigned long long int *)&(x) << 56) | \
