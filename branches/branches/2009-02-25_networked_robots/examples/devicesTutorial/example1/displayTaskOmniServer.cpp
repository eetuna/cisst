...
/* $Id: displayTaskOmniServer.cpp 346 2009-05-12 06:54:49Z adeguet1 $ */


#include "displayTaskOmniServer.h"
#include "displayUI.h"

CMN_IMPLEMENT_SERVICES(displayTaskOmniServer);

displayTaskOmniServer::displayTaskOmniServer(const std::string & taskName, double period):
    mtsTaskPeriodic(taskName, period, false, 500),
    ExitFlag(false)
{
    // to communicate with the interface of the resource
    mtsRequiredInterface * requiredInterface = AddRequiredInterface("LocalRobot");
	...
	if (requiredInterface) {
        requiredInterface->AddEventHandlerWrite(&displayTaskOmniServer::Button1EventHandler, this,
...
void displayTaskOmniServer::Configure(const std::string & CMN_UNUSED(filename))
{
    // define some values, ideally these come from a configuration
    // file and then configure the user interface
}

void displayTaskOmniServer::Startup(void) 
{
    // find the interface which has been connected to our resource port
    mtsDeviceInterface * providedInterface = GetProvidedInterfaceFor("LocalRobot");
...
void displayTaskOmniServer::Run(void)
{
    // process events
...
void displayTaskOmniServer::Button1EventHandler(const prmEventButton & buttonEvent)
{
...
void displayTaskOmniServer::Button2EventHandler(const prmEventButton & buttonEvent)
{
    if (buttonEvent.Type() == prmEventButton::PRESSED) {
