...
int svlFilterImageWindow::Initialize(svlSample* syncInput, svlSample* &syncOutput)
{
	...
	if (GetInput()->GetType() == svlTypeImageRGB || GetInput()->GetType() == svlTypeImageRGBA) {
		...
		{
			svlSampleImageRGBA* img = dynamic_cast<svlSampleImageRGBA*>(syncInput);
			WindowManager->SetClientSize(img->GetWidth(), img->GetHeight(), 0);
		}
		if (PositionSetFlag) {
		...
		if(GetInput()->GetType() == svlTypeImageRGBStereo)
		{
		...
		else if(GetInput()->GetType() == svlTypeImageRGBAStereo)
		{
			svlSampleImageRGBAStereo* stimg = dynamic_cast<svlSampleImageRGBAStereo*>(syncInput);
			WindowManager->SetClientSize(stimg->GetWidth(SVL_LEFT), stimg->GetHeight(SVL_LEFT), 0);
			WindowManager->SetClientSize(stimg->GetWidth(SVL_RIGHT), stimg->GetHeight(SVL_RIGHT), 1);
		}
		if (PositionSetFlag) {
