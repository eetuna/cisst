...
/********************************************/
/* svlVidCapSrcSDIRenderTargetCapture class */
/********************************************/
svlVidCapSrcSDIRenderTargetCapture::svlVidCapSrcSDIRenderTargetCapture():
svlVidCapSrcBase(),
svlRenderTargetBase(),
NumOfStreams(0),
...
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::constructor()" << std::endl;
#endif
    m_videoWidth = 1920;
    m_videoHeight = 1080;
    m_num_streams = 2;
...
svlVidCapSrcSDIRenderTargetCapture::svlVidCapSrcSDIRenderTargetCapture(unsigned int deviceID, unsigned int displayID):
svlVidCapSrcBase(),
svlRenderTargetBase(),
NumOfStreams(0),
InitializedInput(false),
...
bool svlVidCapSrcSDIRenderTargetCapture::SetImage(unsigned char* buffer, int offsetx, int offsety, bool vflip, int index)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::SetImage(): " << index << " (" << offsetx<<"," <<offsety<< ")" <<std::endl;
#endif

	if (SystemID.size() < 0 || SystemID[0] < 0) return false;
...
svlFilterSourceVideoCapture::PlatformType svlVidCapSrcSDIRenderTargetCapture::GetPlatformType()
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::GetPlatformType()" << std::endl;
#endif

	return svlFilterSourceVideoCaptureTypes::NVIDIAQuadroSDI;
}

int svlVidCapSrcSDIRenderTargetCapture::SetStreamCount(unsigned int numofstreams)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::SetStreamCount(" << numofstreams << ")" << std::endl;
#endif
	...
	CaptureProc = new svlVidCapSrcSDIRenderTargetCaptureThread*[SDINUMDEVICES];
	CaptureThread = new osaThread*[SDINUMDEVICES];
	VideoSourceCaptureProc = new svlVidCapSrcSDIRenderTargetCaptureThreadSource*[SDINUMDEVICES];
	VideoSourceThread = new osaThread*[SDINUMDEVICES];
	...
	for (unsigned int i = 0; i < SDINUMDEVICES; i ++) {
		CaptureProc[i] = 0;
		CaptureThread[i] = 0;
		VideoSourceCaptureProc[i] = 0;
		VideoSourceThread[i] = 0;
	}
...
int svlVidCapSrcSDIRenderTargetCapture::GetDeviceList(svlFilterSourceVideoCapture::DeviceInfo **deviceinfo)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::GetDeviceList(...)" << std::endl;
#endif
	...
	if (SDINUMDEVICES > 0) {
		...
		for (sys = 0; sys < SDINUMDEVICES; sys ++) {
			// platform
			deviceinfo[0][sys].platform = svlFilterSourceVideoCaptureTypes::NVIDIAQuadroSDI;
...
int svlVidCapSrcSDIRenderTargetCapture::Open()
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::Open() - Number of video channels = " << NumOfStreams << std::endl;
#endif
	...
	for (unsigned int i = 0; i < NumOfStreams; i ++) {
		...
		//#endif
		ImageBuffer[i] = new svlBufferImage(width*m_resizeRatio, height*m_resizeRatio);
	}
...
void svlVidCapSrcSDIRenderTargetCapture::Close()
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::Close()" << std::endl;
#endif
	...
	//Done in VideoSourceThread::Proc
	for (unsigned int i = 0; i < NumOfStreams; i ++) {
...
int svlVidCapSrcSDIRenderTargetCapture::Start()
{
#if __VERBOSE__ == 1
	...
	Running = true;
	for (unsigned int i = 0; i < SDINUMDEVICES; i ++) {
		CaptureProc[i] = new svlVidCapSrcSDIRenderTargetCaptureThread(i);
		CaptureThread[i] = new osaThread;
		VideoSourceCaptureProc[i] = new svlVidCapSrcSDIRenderTargetCaptureThreadSource(i);
		VideoSourceThread[i] = new osaThread;

		CaptureThread[i]->Create<svlVidCapSrcSDIRenderTargetCaptureThread, svlVidCapSrcSDIRenderTargetCapture*>(CaptureProc[i],
			&svlVidCapSrcSDIRenderTargetCaptureThread::Proc,
			this);
		VideoSourceThread[i]->Create<svlVidCapSrcSDIRenderTargetCaptureThreadSource, svlVidCapSrcSDIRenderTargetCapture*>(VideoSourceCaptureProc[i],
			&svlVidCapSrcSDIRenderTargetCaptureThreadSource::Proc,
			this);//Create<svlVidCapSrcSDIRenderTargetCapture, void*>(this, &svlVidCapSrcSDIRenderTargetCapture::VideoSourceThreadProc, 0);

		if (CaptureProc[i]->WaitForInit() == false) return SVL_FAIL;
		if (VideoSourceCaptureProc[i]->WaitForInit() == false) return SVL_FAIL;
...
svlImageRGB* svlVidCapSrcSDIRenderTargetCapture::GetLatestFrame(bool waitfornew, unsigned int videoch)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::GetLatestFrame(" << waitfornew << ", " << videoch << ")" << std::endl;
#endif
...
int svlVidCapSrcSDIRenderTargetCapture::Stop()
{
#if __VERBOSE__ == 1
	...
	for (unsigned int i = 0; i < SDINUMDEVICES; i ++) {
		...
		if (VideoSourceThread[i]) {
			VideoSourceThread[i]->Wait();
			delete(VideoSourceThread[i]);
			VideoSourceThread[i] = 0;
		}
		if (VideoSourceCaptureProc[i]) {
			delete(VideoSourceCaptureProc[i]);
			VideoSourceCaptureProc[i] = 0;
		}
	}
...
bool svlVidCapSrcSDIRenderTargetCapture::IsRunning()
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::IsRunning()" << std::endl;
#endif

	return Running;
}

int svlVidCapSrcSDIRenderTargetCapture::SetDevice(int devid, int inid, unsigned int videoch)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::SetDevice(" << devid << ", " << inid << ", " << videoch << ")" << std::endl;
#endif
...
int svlVidCapSrcSDIRenderTargetCapture::GetWidth(unsigned int videoch)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::GetWidth(" << videoch << ")" << std::endl;
#endif
...
int svlVidCapSrcSDIRenderTargetCapture::GetHeight(unsigned int videoch)
{
#if __VERBOSE__ == 1
...
int svlVidCapSrcSDIRenderTargetCapture::GetFormatList(unsigned int deviceid, svlFilterSourceVideoCapture::ImageFormat **formatlist)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::GetFormatList(" << deviceid << ", ...)" << std::endl;
#endif
...
int svlVidCapSrcSDIRenderTargetCapture::GetFormat(svlFilterSourceVideoCapture::ImageFormat& format, unsigned int videoch)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::GetFormat(..., " << videoch << ")" << std::endl;
#endif
...
bool svlVidCapSrcSDIRenderTargetCapture::IsCaptureSupported(unsigned int sysid, unsigned int digid)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::IsCaptureSupported(" << sysid << ", " << digid << ")" << std::endl;
#endif
...
bool svlVidCapSrcSDIRenderTargetCapture::IsOverlaySupported(unsigned int sysid, unsigned int digid)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::IsOverlaySupported(" << sysid << ", " << digid << ")" << std::endl;
#endif
...
svlVidCapSrcSDIRenderTargetCaptureThreadSource::svlVidCapSrcSDIRenderTargetCaptureThreadSource(int streamid):
CaptureCount(0)
{
	StreamID = streamid;
	InitSuccess = false;
	...
	mem_left = new ISSI_Endoscope_Image_MemoryMap_Entry;
	memset(mem_left->str, 0, sizeof(ISSI_Endoscope_Image_MemoryMap));

	mem_right = new ISSI_Endoscope_Image_MemoryMap_Entry;
	memset(mem_right->str, 0, sizeof(ISSI_Endoscope_Image_MemoryMap));
#endif
}

svlVidCapSrcSDIRenderTargetCaptureThreadSource::~svlVidCapSrcSDIRenderTargetCaptureThreadSource()
{
#if CISST_SVL_HAS_ISSI 
	delete mem_left; 
	delete mem_right; 
...
void svlVidCapSrcSDIRenderTargetCaptureThreadSource::flip(unsigned char* image, int index)
{
	const unsigned int stride =  m_videoWidth*m_resizeRatio * 3;
	const unsigned int rows = (int)(m_videoHeight*m_resizeRatio) >> 1;
	//unsigned char *flipInput =  (unsigned char *) malloc (m_SDIin.GetWidth() * 3 * m_SDIin.GetHeight());
	//unsigned char *tempOutput =  (unsigned char *) malloc (m_SDIin.GetWidth() * 3 * m_SDIin.GetHeight());
	unsigned char *down = image;
	unsigned char *up = image + (int)(m_videoHeight*m_resizeRatio - 1) * stride;
...
#if CISST_SVL_HAS_ISSI
void svlVidCapSrcSDIRenderTargetCaptureThreadSource::writeToISSI(unsigned char* image, int index, double resampleRatio)
{
	if(index == 0)
	{
		mem_left->data.Image.header.width		= m_videoWidth*resampleRatio;
		mem_left->data.Image.header.height		= m_videoHeight*resampleRatio;
		mem_left->data.Image.header.depth		= 3;
		mem_left->data.Image.header.stereo		= false;
		mem_left->data.Image.header.num_slices	= 1;
			
		std::memcpy((void*)mem_left->data.Image.data, (void*)image, GetImageSize(mem_left->data.Image.header));
			
		m_issi.SetEndoscopeImageMemory(mem_left, 0);
	}else if(index == 1)
	{
		mem_right->data.Image.header.width		= m_videoWidth*resampleRatio;
		mem_right->data.Image.header.height		= m_videoHeight*resampleRatio;
		mem_right->data.Image.header.depth		= 3;
		mem_right->data.Image.header.stereo		= false;
		mem_right->data.Image.header.num_slices	= 1;
			
		std::memcpy((void*)mem_right->data.Image.data, (void*)image, GetImageSize(mem_right->data.Image.header));
			
		m_issi.SetEndoscopeImageMemory(mem_right, 1);
	}
}
#endif

void svlVidCapSrcSDIRenderTargetCaptureThreadSource::ResampleRGBA32toRGB24(unsigned char* overlay, unsigned char* src, unsigned char* dst,
...
{
	unsigned int w1 = 255 + 1;//200+1
	unsigned int w0 = 256 - w1;

    unsigned int i, j;
    unsigned int x1, y1, x2, y2;
    unsigned char *poverlay, *ploverlay, *psrc, *plsrc, *pdst;
    const unsigned int srcstride = srcwidth * 4;
    ...
    for (j = 0; j < dstheight; j ++) {
        ...
        for (i = 0; i < dstwidth; i ++) {
            *pdst = (w0*(poverlay[0])+w1*psrc[2]) >> 8; pdst++;
            *pdst = (w0*(poverlay[1])+w1*psrc[1]) >> 8; pdst++;
            *pdst = (w0*(poverlay[2])+w1*psrc[0]) >> 8; pdst++;
...
void svlVidCapSrcSDIRenderTargetCaptureThreadSource::RGBA32toRGB24(unsigned char* overlay, unsigned char* input, unsigned char* output, const unsigned int pixelcount, int index)
{
	unsigned int w1 = 200 + 1;
	unsigned int w0 = 256 - w1;

    for (unsigned int i = 0; i < pixelcount; i ++) {
        *output = (w0 * (*overlay) + w1 * (*input)) >> 8; output ++; input ++; overlay++;
        *output = (w0 * (*overlay) + w1 * (*input)) >> 8; output ++; input ++; overlay++;
        *output = (w0 * (*overlay) + w1 * (*input)) >> 8; output ++; input += 2; overlay+=2;
    }
}

void svlVidCapSrcSDIRenderTargetCaptureThreadSource::ResampleRGBA24(unsigned char* src, const unsigned int srcwidth, const unsigned int srcheight,
...
void* svlVidCapSrcSDIRenderTargetCaptureThreadSource::Proc(svlVidCapSrcSDIRenderTargetCapture* baseref)
{
	Error = false;
	InitSuccess = true;
	InitEvent.Raise();
	double issiResizeRatio = 0.5;

	for (int i = 0; i < m_num_streams; i++) {
		comprBuffer[i] =  (unsigned char *) malloc (m_videoWidth * 3 *m_videoHeight);
#if CISST_SVL_HAS_ISSI
		issiBuffer[i] =  (unsigned char *) malloc (m_videoWidth*issiResizeRatio * 3 *m_videoHeight*issiResizeRatio);
#endif
		// output to video source
		//baseref->ImageBuffer[i] = new svlBufferImage(m_videoWidth*m_resizeRatio, m_videoHeight*m_resizeRatio);
	}

	unsigned char* ptr;
	while (!baseref->KillThread && baseref->Running) 
	{
		for (int i = 0; i < m_num_streams; i++) 
		{
			if(baseref->CaptureProc[0]->CaptureCount > 0 && baseref->ImageBuffer[i] && baseref->m_overlayBuf[i] && baseref->CaptureProc[0]->m_memBuf[i] )
			{
				unsigned int size=0;
				ptr = baseref->ImageBuffer[i]->GetPushBuffer(size);
				if (!size || !ptr) { /* trouble */ }
				ResampleRGBA32toRGB24(baseref->m_overlayBuf[i], baseref->CaptureProc[0]->m_memBuf[i], ptr ,m_videoWidth,m_videoHeight,m_videoWidth*m_resizeRatio,m_videoHeight*m_resizeRatio,i);
				//RGBA32toRGB24(baseref->m_overlayBuf[i],baseref->CaptureProc[0]->m_memBuf[i], ptr, m_videoWidth*m_videoHeight,i);
				flip(ptr,i);
#if CISST_SVL_HAS_ISSI
				if(CaptureCount == 0)
				{
					ResampleRGBA24(ptr,m_videoWidth,m_videoHeight,issiBuffer[i],m_videoWidth*issiResizeRatio,m_videoHeight*issiResizeRatio);
					writeToISSI(issiBuffer[i],i,issiResizeRatio);
					if(i > 0)
						CaptureCount = 10;
				}else
				{
...
#endif
				if(!baseref->KillThread && baseref->Running) 
	...
	for (unsigned int i = 0; i < m_num_streams; i ++) {
		delete comprBuffer[i];
		comprBuffer[i] = 0;
	}

	return this;
}
//////////////////////////////////////svlVidCapSrcSDIRenderTargetCaptureThread////////////////////////////////////////////

svlVidCapSrcSDIRenderTargetCaptureThread::svlVidCapSrcSDIRenderTargetCaptureThread(int streamid)
{
	StreamID = streamid;
	InitSuccess = false;
	CaptureCount = 0;
...
void* svlVidCapSrcSDIRenderTargetCaptureThread::Proc(svlVidCapSrcSDIRenderTargetCapture* baseref)
{
	m_baseref = baseref;
	Error = false;
	InitSuccess = true;
	...
	// debug
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::ThreadProc(), pitches: " << m_SDIin.GetBufferObjectPitch (0) << ", " << m_SDIin.GetBufferObjectPitch (1) << " width: " << m_SDIin.GetWidth() << " height: " << m_SDIin.GetHeight() << std::endl;

	// Main capture + overlay loop
	baseref->ThreadKilled = false;
	baseref->ThreadReadySignal.Raise();
	...
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::ThreadProc(), pitches: " << pitch0 << ", " << pitch1 << " height: " << height << std::endl;
	for (int i = 0; i < m_SDIin.GetNumStreams (); i++) {
		// overlay from set image
		baseref->m_overlayBuf[i] = (unsigned char *) malloc (m_SDIin.GetWidth() * m_SDIin.GetHeight()*4);
		// input capture frames
		m_memBuf[i] =
			(unsigned char *) malloc (m_SDIin.GetBufferObjectPitch (i) *
			m_SDIin.GetHeight ());

		//#if __VERBOSE__ == 1
		std::cout << "svlVidCapSrcSDIRenderTargetCapture::ThreadProc - Allocate image buffer (" << m_SDIin.GetWidth() << ", " << m_SDIin.GetHeight() << ")" << std::endl;
		//#endif
	...
	while (!baseref->KillThread && baseref->Running) {
		if (CaptureVideo() != GL_FAILURE_NV)
		{
			...
			if(!baseref->KillThread && baseref->Running)
			{
				...
				if(baseref->NewFrameSignal.Wait(0.0005))
				{
					baseref->ThreadReadySignal.Raise();
				}
			}
...
/////////////////////////////////////
void svlVidCapSrcSDIRenderTargetCaptureThread::cudaCheckErrors(char *label)
{
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::cudaInitDevice()
{
	/////////////////////////////////////
	// (1) initialisations:
	...
	if(cerr!=cudaSuccess)
	{
		fprintf(stderr,"failed in cudaGetDeviceCount\n");
		return E_FAIL;
	}
	//get a cuda device that corresponds to the affinity handle of the primary GPU
	cerr = cudaWGLGetDevice(&selectedDevice,m_gpu->getAffinityHandle());	
	if(cerr!=cudaSuccess)
	{
		fprintf(stderr,"failed in cudaWGLGetDevice\n");
		return E_FAIL;
	}
	// now setup the interop	
	cerr = cudaGLSetGLDevice(selectedDevice);
	cudaCheckErrors("cudaGLSetGLDevice");
	return S_OK;	
}

HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::cudaRegisterBuffers()
{	
	for (unsigned int i = 0; i < m_SDIin.GetNumStreams(); i++) {
		cudaGraphicsGLRegisterBuffer(&cudaVideoGraphicsResource[i],m_vidBufObj[i],cudaGraphicsMapFlagsReadOnly);
		cudaCheckErrors("setup");
		cudaGraphicsGLRegisterBuffer(&cudaOutBufferGraphicsResource[i],m_videoFrameBuffer[i],cudaGraphicsMapFlagsReadOnly);	
	}

	return S_OK;
}

HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::cudaUnregisterBuffers()
{
	// Unregister the video capture buffers with CUDA  
	for (unsigned int i = 0; i < m_SDIin.GetNumStreams(); i++) {
		cudaGraphicsUnregisterResource(cudaVideoGraphicsResource[i]);
		cudaCheckErrors("cleanup");
		cudaGraphicsUnregisterResource(cudaOutBufferGraphicsResource[i]);
	}

	return S_OK;
}

HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::setupCUDA()
{
	cudaInitDevice();
	//Setup other cuda buffers here if necessary
	glGenBuffers(m_SDIin.GetNumStreams(), m_videoFrameBuffer);
	...
	for (unsigned int i = 0; i < m_num_streams; i ++) {
		...
		//glGenBuffers(1, &m_CudaOutBuffer);
		//assert(glGetError() == GL_NO_ERROR);
		glBindBuffer(GL_VIDEO_BUFFER_NV, m_videoFrameBuffer[i]);
		assert(glGetError() == GL_NO_ERROR);
		// Allocate required space in video capture buffer
		glBufferData(GL_VIDEO_BUFFER_NV, m_SDIin.GetBufferObjectPitch(i) * m_videoHeight, NULL, GL_STREAM_COPY);
		assert(glGetError() == GL_NO_ERROR);
...
bool svlVidCapSrcSDIRenderTargetCaptureThread::doCuda()
{
	// Map buffer(s) into CUDA
	cudaError_t cerr;
	size_t size;
	int resourcesCount = m_SDIin.GetNumStreams();
	unsigned char *inDevicePtr;
	cerr = cudaGraphicsMapResources(resourcesCount, cudaVideoGraphicsResource,0);
	cudaCheckErrors("map");	
	cerr = cudaGraphicsMapResources(resourcesCount, cudaOutBufferGraphicsResource,0);
	cudaCheckErrors("map");	

	for(unsigned int i=0; i<m_SDIin.GetNumStreams(); i++)
	{
		cerr = cudaGraphicsResourceGetMappedPointer((void **)&inDevicePtr, &size, cudaVideoGraphicsResource[i]);	
		cudaCheckErrors("get mapped pointer");	
		unsigned char *outDevicePtr;
		cerr = cudaGraphicsResourceGetMappedPointer((void **)&outDevicePtr, &size, cudaOutBufferGraphicsResource[i]);	
		cudaCheckErrors("get mapped pointer 2");	
		...
		cudaMemcpy(outDevicePtr,inDevicePtr,m_SDIin.GetBufferObjectPitch(i)*m_videoHeight,cudaMemcpyDeviceToDevice);	
		//
		// End of Dummy CUDA operation
		//
	}
	cerr = cudaGraphicsUnmapResources(resourcesCount, cudaVideoGraphicsResource,0);
	//cuGLUnmapBufferObject(inBuf);
	cudaCheckErrors("unmap");		
	cerr = cudaGraphicsUnmapResources(resourcesCount, cudaOutBufferGraphicsResource,0);
	//cuGLUnmapBufferObject(m_CudaOutBuffer);
	cudaCheckErrors("unmap");	
	return TRUE;
}

bool svlVidCapSrcSDIRenderTargetCaptureThread::ProcessVideo()
{
	doCuda();
	...
	for(unsigned int i=0;i<m_SDIin.GetNumStreams();i++)
	{
		glBindTexture(GL_TEXTURE_RECTANGLE_NV, m_videoTextures[i]); 
		assert(glGetError() == GL_NO_ERROR);
		glBindBuffer(GL_PIXEL_UNPACK_BUFFER, m_videoFrameBuffer[i]);
		assert(glGetError() == GL_NO_ERROR);
		glPixelStorei(GL_UNPACK_ROW_LENGTH,m_SDIin.GetBufferObjectPitch (i)/4);		
		assert(glGetError() == GL_NO_ERROR);
		glTexSubImage2D(GL_TEXTURE_RECTANGLE_NV, 0, 0, 0, m_videoWidth, m_videoHeight,
			GL_RGBA, GL_UNSIGNED_BYTE, 0);
		glGetBufferSubData (GL_PIXEL_UNPACK_BUFFER, 0,
...
void svlVidCapSrcSDIRenderTargetCaptureThread::drawCircle(GLuint gWidth, GLuint gHeight)
{
...
//
void svlVidCapSrcSDIRenderTargetCaptureThread::CalcWindowSize()
{  
...
HWND svlVidCapSrcSDIRenderTargetCaptureThread::SetupWindow(HINSTANCE hInstance, int x, int y, 
													 char *title)
{
	WNDCLASS   wndclass;
	//HWND	   hWnd;
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::Configure(char *szCmdLine[])
{
	int numGPUs;
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::SetupSDIDevices()
{
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::setupSDIinDevices()
{
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::setupSDIoutDevice()
{
#ifdef _DEBUG
...
GLboolean svlVidCapSrcSDIRenderTargetCaptureThread::SetupGL()
{
...
// Initialize OpenGL
//
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::setupSDIinGL()
{
	...
	////////////////////////////////////////
	glGenBuffers(m_SDIin.GetNumStreams(), m_vidBufObj);
	//m_videoHeight = m_SDIin.GetHeight();
	//m_videoWidth = m_SDIin.GetWidth();

	for(unsigned int i = 0; i < m_SDIin.GetNumStreams();i++)
	{
		m_SDIin.BindVideoFrameBuffer(m_vidBufObj[i],GL_RGBA8, i);

		// Allocate required space in video capture buffer
		glBindBuffer(GL_VIDEO_BUFFER_NV, m_vidBufObj[i]);
		assert(glGetError() == GL_NO_ERROR);
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::setupSDIoutGL()
{
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::StartSDIPipeline()
{
	// Start video capture
...
GLenum svlVidCapSrcSDIRenderTargetCaptureThread::CaptureVideo(float runTime)
{
...
GLboolean svlVidCapSrcSDIRenderTargetCaptureThread::DrawOutputScene()
{
	GLuint width;
	GLuint height;
	...
	for(unsigned int i=0;i<m_num_streams;i++)
	{
		...
		drawUnsignedCharImage(m_baseref->m_overlayBuf[i]);
#endif
...
void svlVidCapSrcSDIRenderTargetCaptureThread::drawUnsignedCharImage(unsigned char* imageData)
{
#if __VERBOSE__ == 1
	std::cout << "svlVidCapSrcSDIRenderTargetCapture::drawUnsignedCharImage()" << std::endl;
#endif
...
GLboolean svlVidCapSrcSDIRenderTargetCaptureThread::OutputVideo()
{
	if(!m_bSDIout)
		return GL_FALSE;
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::stopSDIPipeline()
{
	m_SDIin.EndCapture();
	return S_OK;
}

GLboolean svlVidCapSrcSDIRenderTargetCaptureThread::cleanupSDIGL()
{
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::cleanupSDIinGL()
{
	for(unsigned int i = 0; i < m_SDIin.GetNumStreams(); i++)
...
HRESULT svlVidCapSrcSDIRenderTargetCaptureThread::cleanupSDIoutGL()
{
...
void* svlVidCapSrcSDIThread::Proc(svlVidCapSrcSDIRenderTargetCapture* baseref)
{
	...
	unsigned int height = m_SDIin.GetHeight();
	//unsigned int size = pitch0*height;
	std::cout << "svlVidCapSrcSDIThread::Proc(svlVidCapSrcSDI), pitches: " << pitch0 << ", " << pitch1 << " height: " << height << std::endl;
	for (int i = 0; i < m_SDIin.GetNumStreams (); i++) {
		m_memBuf[i] =
			(unsigned char *) malloc (m_SDIin.GetBufferObjectPitch (i) *
			m_SDIin.GetHeight ());
		...
		//#if __VERBOSE__ == 1
		std::cout << "svlVidCapSrcSDIThread::Proc(svlVidCapSrcSDI) - Allocate image buffer (" << m_SDIin.GetWidth() << ", " << m_SDIin.GetHeight() << ")" << std::endl;
		//#endif
